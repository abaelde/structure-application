...
def test_csv_aligned_integration():
    ...
    try:
        ...
        program_df, structures_df, conditions_df, exclusions_df = csv_adapter.read(
            "examples/programs/aviation_axa_xl_2024"
        )
        ...
        print(
            f"   Colonnes REINSURANCE_PROGRAM: {len(program_df.columns)} - {list(program_df.columns)[:3]}..."
        )
        print(
            f"   Colonnes RP_STRUCTURES: {len(structures_df.columns)} - {list(structures_df.columns)[:3]}..."
        )
        print(
            f"   Colonnes RP_CONDITIONS: {len(conditions_df.columns)} - {list(conditions_df.columns)[:3]}..."
        )
        print(
            f"   Colonnes RP_GLOBAL_EXCLUSION: {len(exclusions_df.columns)} - {list(exclusions_df.columns)[:3]}..."
        )
        ...
        (
            reloaded_program_df,
            reloaded_structures_df,
            reloaded_conditions_df,
            reloaded_exclusions_df,
        ) = snowflake_adapter.read(dsn, connection_params)
        ...
        if not missing_cols and not extra_cols:
            ...
            print(
                f"   ⚠️  REINSURANCE_PROGRAM: Colonnes manquantes: {missing_cols}, colonnes supplémentaires: {extra_cols}"
            )

        # RP_STRUCTURES
        original_cols = set(structures_df.columns)
        reloaded_cols = set(reloaded_structures_df.columns) - {
            "PROGRAM_ID"
        }  # Exclure la clé de liaison
        missing_cols = original_cols - reloaded_cols
        extra_cols = reloaded_cols - original_cols
        ...
        if not missing_cols and not extra_cols:
            ...
            print(
                f"   ⚠️  RP_STRUCTURES: Colonnes manquantes: {missing_cols}, colonnes supplémentaires: {extra_cols}"
            )

        # RP_CONDITIONS
        original_cols = set(conditions_df.columns)
        reloaded_cols = set(reloaded_conditions_df.columns) - {
            "PROGRAM_ID"
        }  # Exclure la clé de liaison
        missing_cols = original_cols - reloaded_cols
        extra_cols = reloaded_cols - original_cols
        ...
        if not missing_cols and not extra_cols:
            ...
            print(
                f"   ⚠️  RP_CONDITIONS: Colonnes manquantes: {missing_cols}, colonnes supplémentaires: {extra_cols}"
            )

        # RP_GLOBAL_EXCLUSION
        original_cols = set(exclusions_df.columns)
        reloaded_cols = set(reloaded_exclusions_df.columns) - {
            "PROGRAM_ID"
        }  # Exclure la clé de liaison
        missing_cols = original_cols - reloaded_cols
        extra_cols = reloaded_cols - original_cols
        ...
        if not missing_cols and not extra_cols:
            ...
            print(
                f"   ⚠️  RP_GLOBAL_EXCLUSION: Colonnes manquantes: {missing_cols}, colonnes supplémentaires: {extra_cols}"
            )
        ...
        cursor.execute(
            "SELECT TITLE, UW_LOB, CREATED_AT FROM REINSURANCE_PROGRAM LIMIT 1"
        )
        result = cursor.fetchone()
        if result:
            print(
                f"   ✅ REINSURANCE_PROGRAM: {result[0]} - {result[1]} - Créé: {result[2]}"
            )

        # Vérifier RP_STRUCTURES
        cursor.execute(
            "SELECT COUNT(*), COUNT(DISTINCT INSPER_ID_PRE) FROM RP_STRUCTURES"
        )
        ...
        cursor.execute(
            "SELECT COUNT(*), COUNT(DISTINCT BUSCL_ID_PRE) FROM RP_CONDITIONS"
        )
        count, unique_count = cursor.fetchone()
