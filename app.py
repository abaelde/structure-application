...
st.markdown('<div class="main-header">🏢 Reinsurance Program Analyzer</div>', unsafe_allow_html=True)
st.markdown(
...
if program_file and bordereau_file:
    try:
        with st.spinner("🔄 Loading files..."):
            with tempfile.NamedTemporaryFile(delete=False, suffix=".xlsx") as tmp_program:
                tmp_program.write(program_file.read())
                tmp_program_path = tmp_program.name
            
            with tempfile.NamedTemporaryFile(delete=False, suffix=".csv", mode='w', encoding='utf-8') as tmp_bordereau:
        ...
        available_priority_columns = [col for col in priority_columns if col in display_df.columns]
        
        columns_to_exclude = ['structures_detail']
        other_columns = [col for col in display_df.columns 
                       if col not in available_priority_columns 
        ...
        numeric_columns = ['exposure', 'cession_to_layer_100pct', 'cession_to_reinsurer', 'retained_by_cedant']
        ...
        if FIELDS['INSURED_NAME'] in formatted_df.columns:
            column_config[FIELDS['INSURED_NAME']] = st.column_config.TextColumn("Insured", width="medium")
        ...
        if 'cession_to_layer_100pct' in formatted_df.columns:
            column_config['cession_to_layer_100pct'] = st.column_config.TextColumn("Layer Cession (100%)")
        if 'cession_to_reinsurer' in formatted_df.columns:
            column_config['cession_to_reinsurer'] = st.column_config.TextColumn("Reinsurer Cession")
        if 'retained_by_cedant' in formatted_df.columns:
            column_config['retained_by_cedant'] = st.column_config.TextColumn("Cedant Retention")
        ...
        if FIELDS['INSURED_NAME'] in results.columns and len(results) > 0:
            ...
            policy_result = results[results[FIELDS['INSURED_NAME']] == selected_insured].iloc[0]
            ...
            with col1:
                if 'exposure' in policy_result:
                    st.metric("Cedant Gross Exposure", f"{policy_result['exposure']:,.2f}")
                if 'cession_to_layer_100pct' in policy_result:
                    st.metric("Layer Cession (100%)", f"{policy_result['cession_to_layer_100pct']:,.2f}")
            
            with col2:
                if 'cession_to_reinsurer' in policy_result:
                    st.metric("Reinsurer Cession", f"{policy_result['cession_to_reinsurer']:,.2f}")
                if 'retained_by_cedant' in policy_result:
                    st.metric("Cedant Retention", f"{policy_result['retained_by_cedant']:,.2f}")
            
            if 'structures_detail' in policy_result and policy_result['structures_detail']:
                ...
                for i, struct in enumerate(policy_result["structures_detail"], 1):
                    ...
                    with st.expander(
                        ...
                        if struct.get("predecessor_title"):
                            st.info(f"**Predecessor:** {struct['predecessor_title']} (Inuring)")
                        else:
                        ...
                        if struct.get("applied", False):
                            st.write(f"**Layer Cession (100%):** {struct['cession_to_layer_100pct']:,.2f}")
                            st.write(f"**Reinsurer Share:** {struct['reinsurer_share']:.4f} ({struct['reinsurer_share']*100:.2f}%)")
                            st.write(f"**Cession to Reinsurer:** {struct['cession_to_reinsurer']:,.2f}")
                            ...
                            if struct.get("condition"):
                                ...
                                if struct["type_of_participation"] == PRODUCT.QUOTA_SHARE:
                                    if pd.notna(condition.get('CESSION_PCT')):
                                        params_df_data.append({"Parameter": "CESSION_PCT", "Value": f"{condition['CESSION_PCT']}"})
                                    if pd.notna(condition.get('LIMIT')):
                                        params_df_data.append({"Parameter": "LIMIT_100", "Value": f"{condition['LIMIT']:,.0f}"})
                                
                                elif struct["type_of_participation"] == PRODUCT.EXCESS_OF_LOSS:
                                    rescaling_info = struct.get("rescaling_info")
                                    if rescaling_info:
                                        st.warning(f"⚠️ RESCALED (Retention factor: {rescaling_info['retention_factor']:.2%})")
                                        ...
                                        if pd.notna(condition.get('ATTACHMENT_POINT_100')):
                                            params_df_data.append({
                                                "Parameter": "ATTACHMENT_POINT_100",
        ...
        with col2:
            results_for_export = results.drop(columns=['structures_detail'], errors='ignore')
    ...
    st.info("👆 Please upload a program file (Excel) and a bordereau (CSV) to start the analysis")
