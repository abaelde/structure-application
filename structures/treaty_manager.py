...
class TreatyManager:
    ...
    def select_treaty(self, claim_basis: str, policy_inception_date: str, 
                     calculation_date: str = None) -> Optional[Dict[str, Any]]:
        """
        ...
        if claim_basis == "risk_attaching":
            ...
            raise ValueError(f"claim_basis invalide: {claim_basis}. Doit être 'risk_attaching' ou 'loss_occurring'")
        ...
        if treaty is None:
            print(f"⚠️  Aucun traité trouvé pour l'année {reference_year} (claim_basis: {claim_basis})")
...
def create_treaty_manager_from_directory(directory_path: str, pattern: str = "treaty_{year}.xlsx") -> TreatyManager:
    """
