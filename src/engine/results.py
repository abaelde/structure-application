...
class StructureRun:
    ...
    # --- applicabilité ---
    applied: bool
    reason: Optional[str]              # "out_of_period" | "no_matching_condition" | "already_processed" | None
    ...
    metrics: Dict[str, float] = field(default_factory=dict)  # ex: {"hull_input": 11.25e6, "liab_input": 37.5e6}
    ...
    def to_rows(self) -> List[Dict[str, Any]]:
        ...
        for r in self.structures:
            rows.append({
                ...
                "applied": r.applied,
                "reason": r.reason or "",
                "scope": ";".join(sorted(r.scope_components)) if r.scope_components else "total",
                ...
                **{k: v for k, v in terms_as_dict(r.terms).items()},
                ...
                "reinsurer_signed_share": terms_as_dict(r.terms).get("signed_share"),
