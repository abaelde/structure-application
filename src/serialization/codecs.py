...
def join_multi(values: Optional[List[str]], sep: str = MULTI_VALUE_SEPARATOR) -> Optional[str]:
...
def pandas_to_native(v):
    ...
    if isinstance(v, (float, int)) and pd.isna(v): return None
    if hasattr(pd, 'NA') and v is pd.NA: return None
