...
def test_hull_liability_filtering_aviation():
    ...
    assert abs(qs_all_detail["cession_to_layer_100pct"] - expected_qs_cession) < tolerance
    ...
    assert abs(xol_hull_detail["input_exposure"] - hull_component_of_retained) < tolerance
    ...
    expected_hull_cession = max(0, min(rescaled_limit_hull, hull_component_of_retained - rescaled_attachment_hull))
    assert abs(xol_hull_detail["cession_to_layer_100pct"] - expected_hull_cession) < tolerance
    ...
    liability_component_of_retained = retained_after_qs * (liability_exposure / total_exposure)
    assert abs(xol_liability_detail["input_exposure"] - liability_component_of_retained) < tolerance
    rescaled_attachment_liability = 10_000_000 * qs_retention_factor
    rescaled_limit_liability = 40_000_000 * qs_retention_factor
    expected_liability_cession = max(0, min(rescaled_limit_liability, liability_component_of_retained - rescaled_attachment_liability))
    assert abs(xol_liability_detail["cession_to_layer_100pct"] - expected_liability_cession) < tolerance
