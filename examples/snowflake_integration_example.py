...
def example_program_operations():
    ...
    try:
        ...
        reloaded_program = pm.load(dsn, io_kwargs={"connection_params": connection_params})
...
def example_run_operations():
    ...
    try:
        ...
        print(f"   Runs rechargés: {len(loaded_runs)} runs, {len(loaded_policies)} politiques, {len(loaded_structures)} structures")
...
def main():
    ...
    if connection_params["account"] == "your_account":
        ...
        print("   Configurez les variables d'environnement ou modifiez get_connection_params()")
        return
    ...
    if success_count == total_tests:
        ...
        print("⚠️  Certains tests ont échoué. Vérifiez la configuration et les permissions.")
