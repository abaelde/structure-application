...
def example_claim_basis_usage():
    ...
    treaty_summary = results.groupby('selected_treaty_year').agg({
        'policy_number': 'count',
        'exposure': 'sum',
        'ceded': 'sum',
        'retained': 'sum'
    ...
    treaty_summary.columns = ['Nb_Polices', 'Exposition_Totale', 'Cédé_Total', 'Retenu_Total']
    ...
    if len(no_coverage) > 0:
        ...
        for _, policy in no_coverage.iterrows():
            print(f"     - {policy['policy_number']} (souscription: {policy['policy_inception_date']})")
    else:
